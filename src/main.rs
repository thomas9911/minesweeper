...
fn run_minesweeper() {
    ...
    let mut window: PistonWindow = WindowSettings::new("It\'s mine!", [WIDTH, HEIGHT])
        .exit_on_esc(true)
        .build()
        .unwrap();
    ...
    while let Some(e) = window.next() {
        ...
        window.draw_2d(&e, |c, g| {
            ...
            if win & !lost {
                text::Text::new_color([0.0, 0.0, 0.0, 1.0], scale as u32)
                    .draw(
                        "WINNN!!!",
                        &mut glyphs,
                        &c.draw_state,
                        c.transform.trans(WIDTH as f64 / 4.0, HEIGHT as f64 / 2.0),
                        g,
                    ).unwrap();
            }
            if lost {
...
fn print_character(
    ...
    text::Text::new_color(color, scale as u32)
        .draw(
            ...
            &c.draw_state,
            c.transform.trans(
                scale * (cell.x as f64 + 0.15),
                scale * (cell.y as f64 + 0.85),
            ),
            g,
        ).unwrap();
}
