...
impl Tiles{
    ...
    fn is_legit(&mut self, x: usize, y: usize) -> bool {
        (x < self.width) & (y < self.height) & self.tiles.get(x).is_some() & self.tiles.get(y).is_some()
    }
    ...
    pub fn reveal_around(&mut self, x: u32, y: u32){
        ...
        if (self.tiles[x as usize][y as usize].mines_around == 0) & (self.tiles[x as usize][y as usize].flow_revealed == false) {
                ...
                for p in -1..2{
                    for q in -1..2{
                        if !((p==0)&(q==0)){
                            ...
                            if (a < self.width) & (b < self.height) & self.tiles.get(a).is_some() & self.tiles.get(b).is_some(){
...
impl Tile{
    pub fn new(x: usize, y: usize) -> Tile{
        Tile{x: x, y: y, mine: false, flagged: false, mines_around: 0, revealed: false, flow_revealed: false}
    }
